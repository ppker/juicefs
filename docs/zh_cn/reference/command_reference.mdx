...
|`--session-token=value`|对象存储的临时访问凭证（Session Token），查看[如何设置对象存储](../reference/how_to_set_up_object_storage.md#session-token)以了解更多。|
|`--storage-class=value` <VersionAdd>1.1</VersionAdd>|默认存储类型。|
|`--tier-id=value` <VersionAdd>1.4</VersionAdd>|分层存储 ID（1-3），与 `--tier-sc` 配合使用，用于配置指定 tier 的存储类型。ID `0` 为保留默认层，不可在此处配置。|
|`--tier-sc=value` <VersionAdd>1.4</VersionAdd>|分层存储对应的对象存储类型（例如 `STANDARD_IA`、`INTELLIGENT_TIERING`、`GLACIER_IR`），需与 `--tier-id` 同时使用。|
|`--upload-limit=0`|上传带宽限制，单位为 Mbps (默认：0)|
|`--download-limit=0`|下载带宽限制，单位为 Mbps (默认：0)|
|`--max-uploads=20`|上传并发上限（默认：20）|
|`--max-downloads=200` <VersionAdd>1.4</VersionAdd>|下载并发上限（默认：200）|
...
|-|-|
|`META-URL`|用于元数据存储的数据库 URL，详情查看[「JuiceFS 支持的元数据引擎」](../reference/how_to_set_up_metadata_engine.md)。|
|`--path value`|卷中目录的全路径（用于目录配额）|
|`--uid value`|用户 ID（用于用户配额）|
|`--gid value`|组 ID（用于组配额）|
|`--create`|目录不存在时自动创建（仅对目录配额生效）|
|`--capacity value`|空间硬限制，单位 GiB (默认：0)|
|`--inodes value`|硬限制 inode 数 (默认：0)|
...
|`--repair-dir-mode value` <VersionAdd>1.4</VersionAdd>|修复目录时使用的权限模式，八进制格式（默认值："0755"）|
...
|`--hide-internal` <VersionAdd>1.4</VersionAdd>|在目录列表中隐藏所有内部文件（`.accesslog`、`.stats`、`.config`、`.control`），这些文件仍可通过完整路径访问（默认值：false）。|
...
|`--dry`|仅打印执行计划，不实际拷贝文件。|
|`--enable-checkpoint`|启用 checkpoint，用于断点续传。|
|`--checkpoint-interval=10s`|保存 checkpoint 的时间间隔。|
|`--checkpoint-force-reset`|从头开始同步，并覆盖已有 checkpoint。|
...
```shell
# 为多个 sync 实例接入同一个全局流量控制服务
juicefs sync SRC DST --traffic-control-url http://127.0.0.1:8080/traffic-control
```

* 接口定义：`POST /traffic-control`（以实际 URL 路径为准），`Content-Type: application/json`。
* 请求定义：
  ...
  * `bytes`：本次申请的带宽额度（字节数）。正数表示申请额度，负数表示返还未使用额度。
* 返回值定义：
...
管理分层存储（Storage Tier）。详细介绍参考[「分层存储」](../guide/tiered-storage.md)。
...
| 子命令 | 说明 |
|-|-|
|`list META-URL`|列出当前文件系统所有 tier 配置（id 与存储类型）。|
|`set META-URL PATH`|为指定文件或目录设置 tier。|
|`restore META-URL PATH`|向对象存储发起归档恢复请求（适用于 GLACIER / DEEP_ARCHIVE 等归档类型）。|

#### 参数

| 项 | 说明 |
|-|-|
|`--id value`|tier ID（0-3）。`0` 为默认层（保留值），`1-3` 为可配置层。`tier set` 时必填。|
|`--recursive, -r`|递归处理目标目录下的所有文件和子目录（默认：false）。|
|`--force, -f`|强制将对象重写到 tier 当前映射的存储类型，即使 tier id 未改变（默认：false）。适用于 `--tier-sc` 变更后需要迁移已有对象的场景。|
